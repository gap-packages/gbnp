...
InstallGlobalFunction( LtNP, function(u,v)  
...
GBNP.MkMonicNPTrace:=function(mon) 
    local i,ans,lc;
...
GBNP.AddTrace:=function(trace,le,new,ri,scal) 
    local ans,k,j;
...
GBNP.IsNPMNotZero:=function(np)
    if Length(np[1])=0          # zero 
      or Length(np[1][1])=0   # NP monomial '1'
      or np[1][1][1] > 0 then # NP monomial
        return false;
    # else np[1][1][1] < 0, or np[1][1] is an NPM monomial 
    fi;
    return true;
end;
...
InstallGlobalFunction( NumAlgGensNP, function(np)
...
InstallGlobalFunction( NumAlgGensNPList, function(Lnp)
...
GBNP.NumAlgGensNPmonList:=function(Lnp_mon)
    return Maximum( [GBNP.MaximumZ(
                         List(Lnp_mon, x -> GBNP.NumAlgGensNPMon(x))),
                    0] );
end;
...
InstallGlobalFunction( NumModGensNP, function(npm)
    local ans;
...
InstallGlobalFunction( NumModGensNPList, function(Lnpm)
...
GBNP.NumModGensNPmonList:=function(Lnpm_mon)
    return Maximum([GBNP.MaximumZ(List(
                        Lnpm_mon, x -> GBNP.NumModGensNPMon(x))),
                    0] );
end;
